...
create index IX_42BE0F71 on preving_Sign (groupId, userId);
...
create index IX_54E52EE5 on preving_UserData (userId, groupId);
...
create index IX_A5644187 on preving_activity (companyId, groupId);

create index IX_CA61B8AB on preving_audit (userId);

create index IX_F48A9419 on preving_companyconf (cif[$COLUMN_LENGTH:75$]);
create index IX_B9692139 on preving_companyconf (companyId, groupId);

create index IX_66BB5930 on preving_holiday (companyId, groupId);

create index IX_B2DD4691 on preving_sign (companyId, groupId, userId, startDate);