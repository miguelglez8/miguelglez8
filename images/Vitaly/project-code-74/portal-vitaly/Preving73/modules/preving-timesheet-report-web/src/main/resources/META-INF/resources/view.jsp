...
/*	Calendar cal = Calendar.getInstance();
	...
	PrevingSign sign = null;
	 ...
	 boolean hasWorkCenters = false;
        ...
        boolean isAdminUser = UserGroupRoleLocalServiceUtil.hasUserGroupRole(themeDisplay.getUserId(), themeDisplay.getScopeGroupId(), "Preving Company Site Admin");
	...
	<div class="row">
		...
		<c:if test="<%= hasWorkCenters %>">
		    <div class="col-sm-2">
		        <aui:fieldset label="timesheetreport.view.workCenter">
                                <select name="cmbWorkCenterSelect" id="cmbWorkCenterSelect" class="w-100">
                                    <option value=""></option>
		...
		<div class="col-sm-1">
			<aui:button-row>
				...
				<button type="button" class="btn  btn-primary" onClick="searchTimeSheets()"><liferay-ui:message key="timesheetreport.view.search"/></button>
			</aui:button-row>
		</div>
	</div>
</aui:form>
...
<portlet:resourceURL id="getTimeSheets" var="getTimeSheetsURL"/>
<script>
var tableTimeSheets = null;
var urlAjaxBase = "<%=getTimeSheetsURL.toString()%>" ;
var urlAjaxCallTimeSheet = urlAjaxBase + "&startDate=" + $('#<portlet:namespace/>startDate').val() + "&endDate=" + $('#<portlet:namespace/>endDate').val();
	$(document).ready( function () {
        try{
	     $('#cmbWorkCenterSelect').select2({
            ...
            $("#cmbWorkCenterSelect").select2("val", "x");
        }catch(e){}
		tableTimeSheets = $('#table_id').DataTable( {
			dom: 'Bfrtip',
			ajax: {
                            "url": urlAjaxCallTimeSheet,
             ...
             "columnDefs": [
                             {
                                ...
                                "render": function (data, type, row) {
                                    ...
                                    return '<a onclick="toggle(' + "'.sign-" + row.userId + "-" + row.date.replaceAll("/","") + "')" + '">' + '<liferay-ui:message key="timesheetreport.view.detail"/></a>';
	...
	function searchTimeSheets(){
	    ...
	    urlAjaxCallTimeSheet = urlAjaxBase;
	    if($('#<portlet:namespace/>startDate').val() == "" && $('#<portlet:namespace/>endDate').val() == "") return;
	    urlAjaxCallTimeSheet = urlAjaxBase + "&startDate=" + $('#<portlet:namespace/>startDate').val() + "&endDate=" + $('#<portlet:namespace/>endDate').val();

        if($("#cmbWorkCenterSelect").length  && $("#cmbWorkCenterSelect").val() != "" &&  $("#cmbWorkCenterSelect").val().length > 0){
            urlAjaxCallTimeSheet += "&workCenter=" + $("#cmbWorkCenterSelect").val().join(",");
        }
        if($("#<portlet:namespace/>jobtitle").val() != ""){
            urlAjaxCallTimeSheet += "&jobtitle=" + $("#<portlet:namespace/>jobtitle").val();
        }
        console.log("urlAjaxCallTimeSheet: " + urlAjaxCallTimeSheet)
        tableTimeSheets.ajax.url( urlAjaxCallTimeSheet );
        tableTimeSheets.ajax.reload();
