...
<c:forEach items="<%=scheduledJobs%>" var="schedule">
	...
	%>
	<c:if test="<%=schedule.getGroupName().startsWith("com.preving.liferay.portlet.scheduler.web")%>">
		<div class="row">
			<div class="col-sm-4">
				<p>${schedule.groupName}</p>
			</div>
			<div class="col-sm-1">
				<p>${schedule.storageType}</p>
			</div>
			<div class="col-sm-2">
			...
			<div class="col-sm-1">
				<c:if test="<%=schedule.getGroupName().startsWith("com.preving.liferay.portlet.scheduler.web.scheduler")%>">
					<liferay-portlet:actionURL name="deleteScheduler" var="deleteSchedulerURL" >
						<portlet:param name="groupName" value="${schedule.groupName}" />
						<portlet:param name="storageType" value="${schedule.storageType}" />
					</liferay-portlet:actionURL>
					<a href="<%=deleteSchedulerURL%>"><liferay-ui:message key="delete"/></a>
				</c:if>
			</div>
		</div>
	</c:if>
