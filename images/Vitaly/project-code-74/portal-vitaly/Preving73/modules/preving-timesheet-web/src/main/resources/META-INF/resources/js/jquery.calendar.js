...
*/
//Controlar que mes, etc ...  esta seleccionado
var CALENDARIO_PICKER = (function(){
	return {
		MES : null,
		ANO: null
	}
...
(function ($) {
	...
	function createCalendar(month, year, options) {
		...
		//R@
		console.log("MUEVE MES: " + currentMonth)
		CALENDARIO_PICKER.MES = currentMonth;
		CALENDARIO_PICKER.ANO = year;
		var dia = new Date();
		var dS = new Date(), dE = new Date();
		dS.setDate(1);
		dS.setMonth(currentMonth)
		dS.setFullYear(year);
		dE.setMonth(currentMonth);
		dE.setDate(calendarioPicker.getDaysInMonth(currentMonth, year));
		try{calendarioPicker.getSigns({diaClick: new Date(), inicio: dS, fin: dE}); }catch(e){}
