...
@Component(
        ...
        service = FinalizarDenuncia90diasScheduler.class
)
public class FinalizarDenuncia90diasScheduler extends BaseMessageListener {

    public static Log _log = LogFactoryUtil.getLog(FinalizarDenuncia90diasScheduler.class);
    ...
    protected void doReceive(Message message) throws Exception {
        ...
        String subject  	= LanguageUtil.get(bundle, "scheduler.message.finalizar.subject");
        String body  	= LanguageUtil.get(bundle, "scheduler.message.finalizar.body");

        Calendar denunciaCreateDate = Calendar.getInstance();

        denunciaCreateDate.setTime(date);
        denunciaCreateDate.add(Calendar.DAY_OF_YEAR, -90);

        System.out.println(dateFormatLog.format(new Date()) + " SCHEDULER START ============================================================");
        System.out.println(dateFormatLog.format(new Date()) + " SCHEDULER FinalizarDenuncia90diasScheduler. Poner estado SIN FINALIZAR a las denuncias creadas antes de " + dateFormat.format(denunciaCreateDate.getTime()) + ".");

        List<Denuncia> denuncias = DenunciaLocalServiceUtil.getDenunciasNotFinishedCreatedBeforeDate(denunciaCreateDate.getTime());

        for(Denuncia denuncia:denuncias){

            if(denuncia.getEstado() != 2 && denuncia.getEstado() != 3) {

                denuncia.setEstado(3);
                denuncia.setEndDate(date);

                DenunciaLocalServiceUtil.updateDenuncia(denuncia);

                //TODO Borrar campos no esenciales.
                ...
                */
                ...
                System.out.println(dateFormatLog.format(new Date()) + " La denuncia " + denuncia.getCodigo() + " ha pasado a estado Sin Finalizar. Fecha creacion " + dateFormat.format(denuncia.getCreateDate()));
    ...
    protected void activate(Map<String, Object> properties) throws SchedulerException {
        ...
        try {
            ...
            String cronExpression = "0 0 16 * * ? *"; //Todos los días a las 16:00h
