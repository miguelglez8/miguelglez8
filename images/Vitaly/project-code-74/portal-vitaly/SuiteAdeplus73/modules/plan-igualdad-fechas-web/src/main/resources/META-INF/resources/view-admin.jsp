...
<liferay-ui:success key="fecha-save-success" message="hitos.view.fecha.save.success" />
<liferay-ui:success key="fecha-delete-success" message="hitos.view.fecha.delete.success" />
...
<script>
    ...
    $(document).ready( function () {
        ...
        tabla = $('#tabla_hitos').on('preXhr.dt', function ( e, settings, data ) {
                    ...
                    "columnDefs": [
                        ...
                        },
                        {
                            ...
                            "searchable": false,
                            "render": function (data, type, row) {
                            				var html= '<div class="d-flex justify-content-end"><a class="add-fecha ico-acciones-tabla" data-id="' + data + '" data-date="' + row.fecha + '" href="#">' +
		                                            '<span class="icon-calendar"></span>' +
		                                         '</a></div>';
                            				if(mostrar){
                            					html= '<div class="d-flex justify-content-end"> ' +
			                                			'<a class="add-fecha ico-acciones-tabla" data-id="' + data + '" data-date="' + row.fecha + '" href="#">' +
			                                            '<span class="icon-calendar"></span>' +
			                                         '</a>' +
			                                         '<a data-id="' + data + '" class="delete-fecha ico-acciones-tabla prv-reverseHover-img delete-document" href="#">' +
        ...
        $(document).on('click', '.add-fecha', function(e){
            e.preventDefault();

            var id = $(this).data('id');
            var date = $(this).data('date');

            $('#addDate').removeClass('hide');
            $('#addDate').addClass('show');

            if(date != null && date != undefined && date != "") {
                var dateDate = new Date(date);
                var formatDate = dateDate.toLocaleDateString("es-ES", { year: 'numeric', month: '2-digit', day: '2-digit' })
                $('#<portlet:namespace /><%= PlanIgualdadFechasWebPortletKeys.FECHA_PARAM %>').val(formatDate);
            }
            $('#<portlet:namespace />hitoId').val(id);
        });

        $(document).on('click', '.delete-fecha', function(e){
            e.preventDefault();

            var id = $(this).data('id');

            $('#deteleDate').removeClass('hide');
            $('#deteleDate').addClass('show');

            $('#<portlet:namespace /><%=PlanIgualdadFechasWebPortletKeys.HITOID_PARAM_DELETE%>').val(id);
        });

        $(document).on('click', '.hide-modal', function(e) {
            e.preventDefault();
            $('#addDate').addClass('hide');
            $('#deteleDate').addClass('hide');
        });
