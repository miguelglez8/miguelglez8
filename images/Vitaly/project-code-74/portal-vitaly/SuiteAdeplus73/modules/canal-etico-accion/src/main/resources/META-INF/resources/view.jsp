...
<%
	...
	long compId = (long)request.getAttribute(AccionPortletKeys.COMPANY_ID);
	List<Accion> listAcciones = (List<Accion>) request.getAttribute(AccionPortletKeys.ACCIONES_LIST);
	...
	String registrado  	= LanguageUtil.get(bundle, "accion.view.denuncia.estado.registrado");
	String enProceso  	= LanguageUtil.get(bundle, "accion.view.denuncia.estado.en.proceso");
	String finalizado  	= LanguageUtil.get(bundle, "accion.view.denuncia.estado.finalizado");
	String sinFinalizar = LanguageUtil.get(bundle, "accion.view.denuncia.estado.sin.finalizar");
%>
	...
	<c:if test="<%=!hasPermission && !omniAdminRole%>">
		<div class="formulario">
			<div class="title-group">
				...
				<p><liferay-ui:message key="accion.view.no.company.permission"/></p>
			</div>
		</div>
	</c:if>
		...
		<div class="formulario">
			<div class="title-group">
				...
				<portlet:renderURL var="addUserURL">
					<portlet:param name="mvcPath" value="/accion.jsp"></portlet:param>
				</portlet:renderURL>
				<aui:button-row cssClass="button-holder">
					<aui:button onClick="<%= addUserURL.toString() %>" value="accion.view.new" cssClass="btn btn-outline-primary btn-sm" primary="true"></aui:button>
				</aui:button-row>
			</div>

			<div class="row">
				<fieldset class="input-group-inline mt-3 mb-2">
					<legend><liferay-ui:message key="accion.view.active"/></legend>
			...
			<div class="row">
				<div class="col">
					<table id="table" class="display" style="width:100%">
						<thead>
						<tr>
							<th><liferay-ui:message key="accion.view.name"/></th>
							<th><liferay-ui:message key="accion.view.estado"/></th>
							<th><liferay-ui:message key="accion.view.active"/></th>
							<th><liferay-ui:message key="accion.view.action"/></th>
						</tr>
						</thead>
						<tbody>
						<c:forEach items="<%=listAcciones%>" var="accion">
							<%
								Accion accion = (Accion) pageContext.getAttribute("accion");
								String estado = registrado;
								if(accion.getEstado() == AccionPortletKeys.ESTADO_EN_PROCESO){
									estado = enProceso;
								}else if(accion.getEstado() == AccionPortletKeys.ESTADO_FINALIZADO){
									estado = finalizado;
								}else if(accion.getEstado() == AccionPortletKeys.ESTADO_SIN_FINALIZAR){
									estado = sinFinalizar;
								}

								//System.out.println(accion.getNombre() + " " + accion.getCompId() + ", " + accion.getActivo() + " " + (accion.getActivo()?yes:no));
							%>
							<liferay-portlet:renderURL varImpl="editURL">
								<portlet:param name="accionEditId" value="<%=String.valueOf(accion.getAccionId())%>" />
								<portlet:param name="mvcPath" value="/accion.jsp" />
							</liferay-portlet:renderURL>
							<tr>
								<td>${accion.nombre}</td>
								<td><%=estado%></td>
								<td><%=accion.getActivo()?yes:no%></td>
								<td>
									<c:if test="<%=permissionChecker.isOmniadmin() || accion.getCompId() == compId%>">
	 ...
	 var filterActivo = "<%=yes%>"; // para pasarle el click en el radio de activo (idiomas�?)
	    ...
	    $.fn.dataTable.ext.search.push(
	            function( settings, data, dataIndex ) {               
	                ...
	                if(filterActivo == "<%=all%>"){
	                    cond++;
	                }else if(filterActivo == data[2].trim()){
	                    cond++;
	                }
