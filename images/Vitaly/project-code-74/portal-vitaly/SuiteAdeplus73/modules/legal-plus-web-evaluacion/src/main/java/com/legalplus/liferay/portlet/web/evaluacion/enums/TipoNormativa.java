...
public enum TipoNormativa {
    ...
    public static String getNormativa(String value, ResourceBundle bundle) {
        for (TipoNormativa normativa : values()) {
            if (normativa.getCodigo().equalsIgnoreCase(value)) {
                return LanguageUtil.get(bundle, normativa.getDescripcion());
            }
        }
        return null;
    }
