    ...
    <div class="formulario">
        ...
        <div class="prv-search prv-form">
            ...
            <div class="row align-items-start mb-4">
                ...
                <div class="col-lg-3 col-md-6 col-12">
                    <fieldset>
                        <legend><liferay-ui:message key="calendario.view.legislaciones.evaluar"/></legend>
        ...
        <div class="row">
            <div class="col">
                ...
                <table id="tabla_evaluaciones" class="display" style="width:100%; display:none;">
                    <thead>
                        <tr>
...
$(document).ready( function () {
     tabla = $('#tabla_evaluaciones').on('preXhr.dt', function ( e, settings, data ) {
                    $("#spnCargando").hide();
                        ...
                        },
                        {
                              ...
                              "render": function (data, type, row) {
                                            if (row.prohibido) {
                                                ...
                                                return  '<a class="ico-acciones-tabla" onclick="saveParamsSearch()" href="${evaluacionUrl}?comp=<%= compId %>&legislacion=' + data + '&version=' + row.version + '">' +
    ...
    $.fn.dataTable.ext.search.push(function( settings, data, dataIndex ) {
        ...
        if (text != "") {
            numCond++;
            if (data[2].toLowerCase().match(text) || data[3].toLowerCase().match(text) ||
                data[4].toLowerCase().match(text) || data[5].toLowerCase().match(text) ||
        ...
        if (activo != "") {
            numCond++;
            if (data[14].trim().match(activo)) cond++;
        }

        if (cumplimiento != "") {
            numCond++;
            if(data[7].trim().match(cumplimiento)) cond++;
        }

        if(familiaVal != ""){
            numCond++;
            var todas = '<liferay-ui:message key="contract.view.familia.todas"/>';
            if(data[3].trim().match(familia) || data[3].trim().match(todas)) cond++;
        }

        if(ambitoVal != ""){
            numCond++;
            if(data[4].trim().match(ambito)) cond++;
        }

        if(ccaaVal != ""){
            numCond++;
            if(data[10].match(ccaa)) cond++;
        }

        if(aytoVal != ""){
            numCond++;
            if(data[11].match(ayto)) cond++;
        }

        if (proximas) {
            numCond++;
            if (data[1].trim().match('^$') && data[7].trim().match('^$')) cond++
        }
        ...
        if (evaluacion != "") {
            numCond++;
            arrDate = data[12].split("/");
            dateFilterCol = new Date(arrDate[2],arrDate[1]-1,arrDate[0],"00","00");
            if(data[7].trim().match('^$') && dateFilterCol.getTime() < new Date().getTime()) cond++
        }
    ...
    function saveParamsSearch(){
        ...
        itemSearch.inpSearchEvaluacion1 = document.getElementById("inpSearchEvaluacion1").checked;
        itemSearch.inpSearchEvaluacion2 = document.getElementById("inpSearchEvaluacion2").checked;
        ...
        itemSearch.inpSearchProximas = document.getElementById("inpSearchProximas").checked;
        ...
        itemSearch.proxStartDate = document.getElementById("<portlet:namespace/>proxStartDate").value;
        itemSearch.proxEndDate = document.getElementById("<portlet:namespace/>proxEndDate").value;


        sessionStorage.setItem("searchParams_calendarioLegislaciones", JSON.stringify(itemSearch));
    ...
    //Recuperar los filtros
        $(document).ready(function(){
            ...
            if(sessionStorage.getItem("searchParams_calendarioLegislaciones") != null && sessionStorage.getItem("searchParams_calendarioLegislaciones") != "null"){
                itemSearch = JSON.parse(sessionStorage.getItem("searchParams_calendarioLegislaciones"));
                if(itemSearch.search && itemSearch.search != "" ) document.getElementById("search").value = itemSearch.search;
                 ...
                 if(itemSearch.inpSearchProximas) document.getElementById("inpSearchProximas").checked = true;

                if(itemSearch.inpSearchCumplimiento1){
                    document.getElementById("inpSearchCumplimiento1").checked = true;
                    advancedSearch = true;
                }
                if(itemSearch.inpSearchCumplimiento2){
                    document.getElementById("inpSearchCumplimiento2").checked = true;
                    advancedSearch = true;
                }
                if(itemSearch.inpSearchCumplimiento3){
                    document.getElementById("inpSearchCumplimiento3").checked = true;
                    advancedSearch = true;
                }
                if(itemSearch.inpSearchCumplimiento4){
                    document.getElementById("inpSearchCumplimiento4").checked = true;
                    advancedSearch = true;
                }

                if(itemSearch.inpSearchActive1){
                    document.getElementById("inpSearchActive1").checked = true;
                    advancedSearch = true;
                }
                 if(itemSearch.inpSearchActive2){
                    document.getElementById("inpSearchActive2").checked = true;
                    advancedSearch = true;
                }
                 if(itemSearch.inpSearchActive3){
                    document.getElementById("inpSearchActive3").checked = true;
                    advancedSearch = true;
                }
                ...
                if(itemSearch.proxStartDate){
                    document.getElementById("proxStartDate").value = itemSearch.proxStartDate;
                    document.getElementById("proxStartDate").dispatchEvent(event);
                    advancedSearch = true;
                }
                if(itemSearch.proxEndDate){
                    document.getElementById("proxEndDate").value = itemSearch.proxEndDate;
                    document.getElementById("proxEndDate").dispatchEvent(event);
                    advancedSearch = true;
                }
