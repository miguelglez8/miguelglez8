			...
			<div class="row">
				<fieldset class="input-group-inline mt-3 mb-2">
					...
					<div class="custom-control custom-radio mr-4">
						<input class="form-check-input" name="inpSearchActive" id="inpSearchActive1" value="Si" type="radio" class="inpSearchActive" checked onChange="tabla.draw();" />
				...
				<div class="col">
					<table id="table_company" class="display" style="width:100%">
						...
						<c:forEach items="<%=companyList%>" var="comp">
							<%
								Comp companyDetail = (Comp) pageContext.getAttribute("comp");
							%>
							<liferay-portlet:renderURL varImpl="editURL">
								<portlet:param name="companyEditId" value="<%=String.valueOf(companyDetail.getCompId())%>" />
								<portlet:param name="mvcPath" value="/company.jsp" />
							</liferay-portlet:renderURL>
							<tr>
								<td>${comp.name}</td>
								<td>${comp.cif}</td>
								<td><%=AdeplusPermissionCompanyUtil.getCompanyApplicationNames(companyDetail.getCompId())%></td>
								<td><%= (Validator.isNotNull(companyDetail.getDeleteDate()) && companyDetail.getDeleteDate().before(calendar.getTime())) ? no : yes %></td>
								<td><%=UserCompanyLocalServiceUtil.getUsersCountFromCompany(companyDetail.getCompId())%></td>
								<td data-sort="<%=Validator.isNotNull(companyDetail.getCreateDate())?dateFormatSearch.format(companyDetail.getCreateDate()):""%>">
									<%=Validator.isNotNull(companyDetail.getCreateDate())?dateFormat.format(companyDetail.getCreateDate()):""%>
								</td>
								<td data-sort="<%=Validator.isNotNull(companyDetail.getDeleteDate())?dateFormatSearch.format(companyDetail.getDeleteDate()):""%>">
									<%=Validator.isNotNull(companyDetail.getDeleteDate())?dateFormat.format(companyDetail.getDeleteDate()):""%>
								</td>
								<td>
