...
public class GoCuestionarioRenderCommand implements MVCRenderCommand {
    ...
    public String render(RenderRequest renderRequest, RenderResponse renderResponse) throws PortletException {
        ...
        if (PlanIgualdadRoleUtil.isAdministradorRole(themeDisplay.getCompanyId(), themeDisplay.getUser())
            ...
            compId = request.getParameter(PlanIgualdadCuestionarioPortletKeys.QUERY_PARAM);

            if (Validator.isNull(compId)) compId = ParamUtil.getString(renderRequest, PlanIgualdadCuestionarioPortletKeys.COMPID_PARAM);
        ...
        Map<Long, List<Pregunta>> secciones = PreguntaLocalServiceUtil.fetchSecciones();
        List<Pregunta> preguntas = PreguntaLocalServiceUtil.getPreguntas(QueryUtil.ALL_POS, QueryUtil.ALL_POS);
        List<Respuesta> respuestas = RespuestaLocalServiceUtil.fetchRespuestas(Long.parseLong(compId), version.getVersionId());

        renderRequest.setAttribute(PlanIgualdadCuestionarioPortletKeys.SECCIONES, secciones);
        renderRequest.setAttribute(PlanIgualdadCuestionarioPortletKeys.PREGUNTAS, preguntas);
        renderRequest.setAttribute(PlanIgualdadCuestionarioPortletKeys.RESPUESTAS, respuestas);
        renderRequest.setAttribute(PlanIgualdadCuestionarioPortletKeys.COMPID_PARAM, Long.parseLong(compId));
        renderRequest.setAttribute(PlanIgualdadCuestionarioPortletKeys.VERSIONID_PARAM, version.getVersionId());
