...
public class  Realm implements Serializable{
    ...
    public Realm(String url, String realmName, String userAdmin, String passAdmin, String secret, String clientId){
        ...
        try{
            ...
            _log.info("+ kc.serverInfo() : " + kc.serverInfo());
    ...
    public void createUser(UserRepresentation userPreving){
        ...
        _log.info("KC PASO 5.1.1");
        _log.info(result.getStatusInfo());
        ...
        List<GroupRepresentation> lstGroupRepresentation = kc.realm(this.realmName).groups().groups();
        _log.info("KC PASO 5.1.2");
        //Start MEJORA
        ...
        for(int x = 0; userPreving.getGroups() != null && x < userPreving.getGroups().size(); x++){
            ...
            for(int i = 0; i < lstGroupRepresentation.size() ;i++){
                //grpRepresentation = (GroupRepresentation)kc.realm(this.realmName).groups().groups().get(i);
                grpRepresentation = lstGroupRepresentation.get(i);
        ...
        lstGroupRepresentation = null;
        _log.info("KC PASO 5.1.4");
    ...
    public boolean updateUserWithoutGroups(UserRepresentation userPreving, long compId, JSONArray listApp){
        boolean isOk = false;
        Response result = null;
        ...
        try{
           ...
           //this.setRolesUser_v2(userPreving,userResource, compId, listApp);
    ...
    public boolean updateUser(UserRepresentation userPreving){
        ...
        try{
            ...
            /*
            _log.info("userPreving.getEmail(): " + userPreving.getEmail());
            _log.info("userPreving.getGroups().size(): " + userPreving.getGroups().size());
            for(String grp : userPreving.getGroups()){
                _log.info("getGroups(): " + grp);
            }
            _log.info("userPreving.getGroups().size(): " + userPreving.getRealmRoles().size());
            for(String roles : userPreving.getRealmRoles()){
                _log.info("getGroups(): " + roles);
            }
            for (Map.Entry  entry : userPreving.getAttributes().entrySet()) {
                _log.info("clave=" + entry.getKey() + ", valor=" + entry.getValue());
                try{
                    List<String> lst = (List<String>)entry.getValue();
                    for(String str : lst){
                        _log.info("valor de la lst:" + str);
                    }
                }catch(Exception e){
                    _log.info("error, null o vacia");
                }
            }*/
    ...
    public boolean isExistUser(UserRepresentation userPreving){
        ...
        for(UserRepresentation user : u){
            if(user.getEmail().equalsIgnoreCase(userPreving.getEmail())){
                isExist = true;
                break;
            }
        }
    ...
    public UserRepresentation getUserFromKCwithOutRoles(String email){
    ...
    public void setUserDisabled(String email){
        UserRepresentation user = this.getUserFromKC(email);
        if(Validator.isNull(user) && !user.isEnabled()) return;
    ...
    public void setRolesUser(UserRepresentation userPreving, UserResource userResource){
        try{
            ...
            for (int i = 0; userPreving.getRealmRoles() != null && i < userPreving.getRealmRoles().size(); i++) {
                ...
                this.createRole_v2(userPreving.getRealmRoles().get(i).toString(), rolesResource, roleList );
                roleResource = rolesResource.get(userPreving.getRealmRoles().get(i).toString());
                try{
                ...
                try{
                    testerRealmRole = rolesResource.get(userPreving.getRealmRoles().get(i).toString()).toRepresentation();
    ...
    //MEJORA
    private  void createRole_v2(String roleName, RolesResource rolesResource, List<RoleRepresentation> roleList ) {
        try{
            //RolesResource rolesResource =  kc.realm(this.realmName).roles();
            RoleResource roleResource = null;
