...
public class AdeplusUserUtilMulti {

    private static final int PASSWORD_LENGTH = 8;

    private static Log _log = LogFactoryUtil.getLog(AdeplusUserUtilMulti.class);
    ...
    private static User createNewUserLiferay(long creatorUserId, long companyId, String portalURL, String pathMain,
        ...
        try {
            ...
            _log.info("Error al crear/updatear al usuario con email "+email);
            _log.error(e);
        }
    ...
    private static boolean isLimitUserOrAdminByApp(long compId, AppPermission appPermission, boolean isSearchAdmin){
        ...
        if (usersUsers.size() > 0 && usersMap.get(appPermission.getApplication().getApplicationId()) >= countTotal) {
            return true;
        }
    ...
    public static boolean isEnabledSelectApp(long compId, Application app){

        boolean isEnabledUsers = true;
        boolean isEnabledAdmin = true;

        AppPermission appPermission = null;
        if(!app.getLimitUsers()){
        ...
        if(!isEnabledAdmin && !isEnabledUsers) return false;
        return true;
    }

    public static String isValidLimitsApp(long companyId,
                                          long compId,
                                          boolean isAdmin,
                                          User user,
        ...
        for( UserCompany u : usersUsers){
            apps = UserApplicationClientLocalServiceUtil.getListApplicationsFromUser(u.getUserId(), compId);

            for(Long ua : apps){

                if(usersMap.containsKey(ua)){
                    usersLong =  (usersMap.get(ua)).longValue() ;
                    usersLong++;
                    usersMap.put(ua, usersLong);
                }else{
                    usersMap.put(ua, 1L);
                }
                //soy yo uno de los usuarios
                if(Validator.isNotNull(user)  && u.getEmail().equalsIgnoreCase(user.getEmailAddress())) {
                    usersLong = (usersMap.get(ua)).longValue();
                    usersLong--;
                    usersMap.put(ua, usersLong);
                }
        ...
        for( UserCompany u : usersAdmins){
            ...
            for(Long ua : apps){
                _log.info("idAPP: "+ua);
                if(adminsMap.containsKey(ua)){
                    adminsLong =  (adminsMap.get(ua)).longValue() ;
                    adminsLong++;
                    adminsMap.put(ua, adminsLong);
                }else{
                    adminsMap.put(ua, 1L);
                }
                //soy yo uno de los usuarios
        ...
        for(AppPermission app : appPermissions){
            ...
            //Si la aplicación esta dentro del array de apps esta checked
            if (isAdmin && !app.getApplication().getLimitAdmins()) { // false significa limitado, true ilimitado
                ...
                if(Validator.isNotNull(usersMap.get(app.getApplication().getApplicationId()))
                        && (usersMap.get(app.getApplication().getApplicationId()) >= app.getApplication().getLimitNumUsers() || app.getApplication().getLimitNumUsers() == 0 )){
    ...
    public static String isValidLimitsApp(long companyId,
                                           ...
                                           User user,
                                           List<AppPermission> appPermissions,
                                           JSONArray jsonContratosApps){

        _log.info("VALIDANDO LIMITES v2 por contrato");
        String result = "";
        JSONObject jsonContrato = null, jsonApp = null;
        JSONArray jsonAllApps = null;
        Map<String,String> mapUsers = new HashMap<String,String>();
        long clienteId = 0, contratoId = 0;
        int countUser = 0, countAdmins = 0;
        for(int i = 0; i < jsonContratosApps.length(); i++){ //recorrer contratos seleccionados y obtener users/admin totales
            jsonContrato = (JSONObject) jsonContratosApps.get(i);
            clienteId = Long.parseLong(jsonContrato.getString("cliente"));
            contratoId = Long.parseLong(jsonContrato.getString("contrato"));

            jsonAllApps = jsonContrato.getJSONArray("apps");

            for(int x = 0; x < jsonAllApps.length(); x++){
                jsonApp = jsonAllApps.getJSONObject(x);
                _log.info("companyId: " + companyId + " / clientId: " + clienteId + " / contratoId: " + contratoId + " / nombre: " + jsonContrato.getString("nombre") + " / appId: " + jsonApp.getString("appId"));
                countUser = UserApplicationClientLocalServiceUtil.getUserCountByContracApp(clienteId,contratoId,Long.parseLong(jsonApp.getString("appId")),(Validator.isNotNull(user))? user.getUserId() : null);
                _log.info("Usuarios (countUser): " +  countUser);
                countAdmins = UserApplicationClientLocalServiceUtil.getAdminsCountByContracApp(clienteId,contratoId,Long.parseLong(jsonApp.getString("appId")),(Validator.isNotNull(user))? user.getUserId() : null);
                _log.info("Admin (countAdmins): " + countAdmins);

                if( !(result = isOverLimitByApp(appPermissions, Long.parseLong(jsonApp.getString("appId")), countUser, countAdmins, isAdmin)).isEmpty() ){
                    break;
                }
            }
    ...
    private  static String  isOverLimitByApp (List<AppPermission> appPermissions, long appId,  int numUser,  int numAdmins, boolean isAdmin){
        String result = "";
        for(AppPermission app : appPermissions){
            if(app.getApplication().getApplicationId() == appId){
                if(!isAdmin){ //comprobamos añadiendo como usuario
                    if(!app.getApplication().getLimitUsers() && app.getApplication().getLimitNumUsers() != 0 ){ //tiene limites usuarios -> false = limitado
                        numUser++;
                        if(numUser > app.getApplication().getLimitNumUsers()){
                            result = "USER: " + app.getApplication().getName();
                            break;
                        }

                    }
                }else{//comprobamos añadiendo como administrador
                    if(!app.getApplication().getLimitAdmins() && app.getApplication().getLimitNumAdmins() != 0 ){ //tiene limites admins -> false = limitado
                        numAdmins++;
                        if(numAdmins > app.getApplication().getLimitNumAdmins()){
                            result = result = "ADMIN: " + app.getApplication().getName();
                            break;
                        }
                    }
                }
    ...
    public static User updateUser( long companyId, long userId, long compId, String name, String lastName, String nif, String email, String phone,
        ...
        if(Validator.isNotNull(user)) {
            ...
            try {
                AdeplusKeycloakMultiUtil.updateUserKeycloakMulti(user.getUserId(), compId, name, lastName, nif, email, phone, isAdmin, getDateFromString(dateEnd), JSONFactoryUtil.createJSONArray());
            }catch (Exception e){
                _log.error("error al actualizar en keycloack el error es: "+e.getMessage());
            }
    ...
    //Es exactamente el mismo que el anterior pero sin la llamada al conector de KC
    public static User updateUserApi( long companyId, long userId, long compId, String name, String lastName, String nif, String email, String phone,
                                   boolean isAdmin, String dateEnd, Locale locale){
    ...
    public static void createUserKeycloak(long companyId,
        ...
        _log.debug("createUserKeycloak: " + userLiferay.getUserId());
        AdeplusKeycloakMultiUtil.operationUserMulti(true,
                ...
                nif, phone, pass, endDate, isAdmin, isNewEmpresa, JSONFactoryUtil.createJSONArray());
    ...
    public static void createUserKeycloak(long companyId,
        ...
        AdeplusKeycloakMultiUtil.operationUserMulti(true,
                companyId, userLiferay.getUserId(), userLiferay.getEmailAddress(), userLiferay.getFirstName(), userLiferay.getLastName(),
                nif, phone, pass, endDate, isAdmin, isNewEmpresa, listAppEmpresa);

    }

    private static JSONArray composeJsonApps(JSONArray lstEmpresa, JSONArray lstUser){ //devuelve el jsonArray de 'apps' sin las apps que no estén a niveld de empresa
        boolean isIn  = false;
        JSONArray compose = JSONFactoryUtil.createJSONArray();
        JSONObject jsonAppUser = null, jsonAppEmpresa = null;
        for(Object appUser: lstUser){
            jsonAppUser = (JSONObject) appUser;
            for(Object appEmpresa: lstEmpresa){
                isIn = false;
                jsonAppEmpresa = (JSONObject) appEmpresa;
                if(jsonAppUser.getString("appId").equals(jsonAppEmpresa.getString("appId"))){
                    isIn = true;
                    break;
                }
            }
            if(isIn){ // si esta
                _log.info("JSON COMPUESTO  composeJsonApps  jsonAppUser : " + jsonAppUser.toJSONString());
                compose.put(jsonAppUser);
            }
        }

        _log.info("JSON COMPUESTO  composeJsonApps  compose : " + compose.toJSONString());
        return compose;
    }

    private static JSONArray getJsonAppsByUserApplicationClient(long clientId, long contract, List<UserApplicationClient> lst) {
        JSONArray result = JSONFactoryUtil.createJSONArray();
        try {
            for (UserApplicationClient userApplicationClient : lst) {
                if (userApplicationClient.getClientId() == clientId && userApplicationClient.getContractId() == contract) {
                    result = JSONFactoryUtil.createJSONArray(userApplicationClient.getApplicationId());
                    break;
                }
            }
        }catch(Exception e){
    ...
    public static List<Comp> getCompaniesActiveFromUser(long userId){
        ...
        if(userId > 0) {
            ...
            for (UserCompany uc : userCompanyList) {
                Comp comp = CompLocalServiceUtil.fetchComp(uc.getCompId());
    ...
    public static String getCompaniesActiveNamesFromUser(long userId){
        String result = "";
        List<Comp> companiesActiveFromUser = getCompaniesActiveFromUser(userId);
        int conta = 0;
        for(Comp comp:companiesActiveFromUser){
            result += comp.getName();
            conta++;
            if(companiesActiveFromUser.size()>conta){
                result += ", ";
            }
        }
    ...
    public static String getCompaniesActiveNameHtmlFromUser(long userId){
        String result = "";
        List<Comp> companiesActiveFromUser = getCompaniesActiveFromUser(userId);

        for(Comp comp:companiesActiveFromUser){
            result += "<p>" + comp.getName() + "</p>";
    ...
    public static User createUserMulti(long creatorUserId, long companyId, String portalURL, String pathMain,
                                       String name, String lastName, String nif, String email, String passwordRandon, String phone, boolean isAdmin,
                                       String dateEnd, long compId, Locale locale){
        ...
        if(Validator.isNotNull(user)){
            ...
            //Add the comp to user in table usercompany.
            AdeplusUserUtilMulti.setUserCompanyClient(user, compId);
            //Set the phone.
            saveUserPhone(user, phone);


            /*AdeplusKeycloakUtil.createUserKeycloak(companyId,
					user,
					nif,
					phone,
					passwordRandon,
            ...
            //Enable or Disable by dateEnd
            //enableDisableUserByDateStr(user, compId, dateEnd);
    ...
    public static User updateUserMulti(long companyId, long userId, long compId, String name, String lastName, String nif, String email, String phone,
                                       boolean isAdmin, String dateEnd, Locale locale, JSONArray listApp){
        ...
        if(Validator.isNotNull(user)) {
            ...
            //Update user in keycloak
            try {
                AdeplusKeycloakMultiUtil.updateUserKeycloakMulti(user.getUserId(), compId, name, lastName, nif, email, phone, isAdmin, getDateFromString(dateEnd),listApp);
            }catch (Exception e){
                _log.error("error al actualizar en keycloack el error es: "+e.getMessage());
            }
            AdeplusAuditUtil.addAudit(user.getCompanyId(), user.getGroupId(), 0, AdeplusAuditPortletKeys.AUDIT_USER_UPDATE, "Updated user " + user.getEmailAddress() + ".");
    ...
    public static void setUserCompanyClient(User user, long compId){
        if(Validator.isNotNull(user) && compId > 0){
            UserCompany userCompany = UserCompanyLocalServiceUtil.getUserCompany(user.getUserId(), compId);
            if(Validator.isNull(userCompany)){
                UserCompanyPK pk = new UserCompanyPK(user.getUserId(), compId);
