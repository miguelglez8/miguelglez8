...
public class RealmBackground implements Serializable{
    ...
    private static Log _log = LogFactoryUtil.getLog(RealmBackground.class);
    ...
    public RealmBackground(){
        ...
        try {
            ...
            this.kc = KeycloakBuilder.builder().serverUrl(URL).realm(REALNAME).username(USERNAME)
                    .password(PASS).clientSecret(SECRET).clientId(CLIENT_ID)
            ...
            _log.error("+ (error) kc.serverInfo() : " + kc.serverInfo());
    ...
    public synchronized void createUser(UserRepresentation userPreving){
        ...
        _log.debug(result.getStatusInfo());
